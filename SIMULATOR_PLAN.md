...
- **UI Framework**: Bootstrap 5 (responsive layout, components, navigation)
...
Set up the project structure, HTML layout with Bootstrap 5, and basic styling.

### Files to Create

```
app/
├── index.html              # Main HTML structure with Bootstrap
├── css/
...
1. **Create index.html**
   ...
   - Navbar with:
     - Project title/logo
     - Challenge selector dropdown (Bootstrap dropdown)
     - Help/documentation link
   - Main layout using Bootstrap Grid:
     - Left column (col-lg-7): ACE editor container
     - Right column (col-lg-5): Canvas arena + controls
   - Control bar (Bootstrap button group): Run, Stop, Step, Reset
   - Speed slider (Bootstrap range input, 1x - 20x)
   - Debug output panel (Bootstrap collapse/accordion)
   - Ultrasonic distance display (Bootstrap badge/alert, always visible)
   - Status bar (Bootstrap alert for success/fail indicator)
   - Footer with challenge info

2. **Create style.css**

   - Bootstrap variable overrides (colors, spacing)
   - Dark theme for editor area
   - Canvas container sizing
   - Custom button colors for Run/Stop/Reset
...
3. **Create app.js**
   ...
   - Bootstrap component initialization (tooltips, dropdowns)
...
- `navbar` - Top navigation with challenge selector
- `dropdown` - Challenge selection menu
- `container-fluid` / `row` / `col-*` - Responsive grid layout
- `btn` / `btn-group` - Control buttons
- `form-range` - Speed slider
- `collapse` / `accordion` - Debug panel toggle
- `badge` / `alert` - Status indicators
- `tooltip` - Help hints on controls

### Deliverables

- [ ] Functional HTML page with Bootstrap layout
- [ ] Responsive navbar with challenge dropdown
- [ ] Styled panels and controls using Bootstrap components
- [ ] Placeholder canvas area
- [ ] Responsive viewport scaling (Bootstrap breakpoints)
...
1. **Integrate ACE Editor**
   ...
   - Fit within Bootstrap column container
...
4. **Starter Code Loading**
   ...
   - Bootstrap modal to confirm before overwriting saved code
...
3. **Maze Selector UI**
   ...
   - Bootstrap dropdown to select maze difficulty/number
   - Bootstrap cards for preview thumbnails (optional)
...
1. **Gamepad UI**
   ...
   - D-pad using Bootstrap buttons (Up, Down, Left, Right)
   - Bootstrap button group for directional controls
   - Start/Select buttons (Bootstrap btn-outline)
   - Positioned below arena using Bootstrap grid for Challenge 7
...
1. **Panel Features**
   ...
   - Scrollable text area (Bootstrap `overflow-auto`)
   - Monospace font (`font-monospace` utility)
   - Auto-scroll to bottom
   - Clear button (Bootstrap btn-sm)
   - Collapsible/expandable (Bootstrap collapse component)
...
1. **Responsive Design**
   ...
   - Test on various screen sizes using Bootstrap breakpoints
   - Mobile-friendly (though primarily desktop)
   - Bootstrap responsive utilities for panel visibility

2. **Accessibility**

   - Keyboard navigation (Bootstrap focus states)
   - ARIA labels on custom components
   - Screen reader labels

3. **Loading States**

   - Bootstrap spinner during Skulpt initialization
   - Disable buttons until ready (Bootstrap disabled state)

4. **Help/Documentation**

   - Bootstrap tooltips on controls
   - Bootstrap offcanvas or modal for help documentation
